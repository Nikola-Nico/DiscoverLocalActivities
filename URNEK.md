...
|---|---|---|---|
| `lat` | float | Не | Географска ширина на корисникот |
| `lon` | float | Не | Географска должина на корисникот |
| `radius_km` | float | Не | Радиус на пребарување во километри (default: 5.0) |
| `category` | string | Не | Филтер по категорија на активност |
| `open_now` | boolean | Не | Ако `true`, враќа само тековно отворени активности |
| `limit` | integer | Не | Број на резултати (default: 20) |
| `offset` | integer | Не | Почетна позиција за пагинација (default: 0) |
...
[
  {
    "id": 1,
    "name": "Градски парк",
    "description": "Централен парк во градот со можности за шетање и рекреација.",
    "category": "парк",
    "latitude": 41.9981,
    "longitude": 21.4254,
...
- **Можни статус кодови:**
  - `200 OK` — Успешно вратена листа (може да биде и празна листа `[]`)
  - `422 Unprocessable Entity` — Невалидни query параметри (на пр. `lat` не е број)
---
...
- **Можни статус кодови:**
  - `200 OK` — Активноста е пронајдена
  - `404 Not Found` — Активност со дадено ID не постои
---
...
{
  "name": "Кафуле Центар",
  "description": "Пријатно кафуле во центарот на градот.",
  "category": "кафуле",
  "latitude": 41.9965,
  "longitude": 21.4314,
...
- **Можни статус кодови:**
  - `201 Created` — Активноста е успешно креирана
  - `422 Unprocessable Entity` — Валидациска грешка (задолжително поле недостасува или е невалидно)
---
...
- **Можни статус кодови:**
  - `200 OK` — Корисникот е пронајден
  - `404 Not Found` — Корисник со дадено ID не постои
---
...
- **Можни статус кодови:**
  - `201 Created` — Корисникот е успешно креиран
  - `422 Unprocessable Entity` — Валидациска грешка (невалиден email, недостасува поле)
---
...
| `limit` | integer | Не | Број на препораки (default: 10) |
...
[
  {
    "id": 5,
    "name": "Спортски центар Борис Трајковски",
    "category": "спорт",
    "distance_km": 2.3,
    "rating": 4.7,
    "score": 0.91
...
- **Можни статус кодови:**
  - `200 OK` — Успешно вратени препораки
  - `404 Not Found` — Корисник со дадено `user_id` не постои
  - `422 Unprocessable Entity` — Невалидни параметри
---
...
- **Можни статус кодови:**
  - `200 OK` — Работното време е вратено
  - `404 Not Found` — Активност со дадено ID не постои
---
...
- **Можни статус кодови:**
  - `201 Created` — Работното време е успешно додадено
  - `422 Unprocessable Entity` — Невалидни вредности (на пр. `close_time` пред `open_time`)
---
...
| `email` | VARCHAR | Unique, Not Null | Електронска адреса (служи и за идентификација) |
| `password` | VARCHAR | Not Null | Лозинка на корисникот |
| `latitude` | FLOAT | | Географска ширина на корисникот (за препораки) |
| `longitude` | FLOAT | | Географска должина на корисникот (за препораки) |
| `created_at` | TIMESTAMP | Default: now() | Датум и час на регистрација |
...
| `description` | TEXT | | Текстуален опис на активноста |
| `category` | VARCHAR | | Категорија (на пр. парк, кафуле, спорт) |
| `latitude` | FLOAT | Not Null | Географска ширина на локацијата |
| `longitude` | FLOAT | Not Null | Географска должина на локацијата |
| `address` | VARCHAR | | Физичка адреса на активноста |
| `rating` | FLOAT | | Просечен рејтинг (0.0 – 5.0) |
| `popularity_score` | INT | Default: 0 | Популарност (број на посети/интеракции) |
...
| `open_time` | TIME | | Час на отворање |
| `close_time` | TIME | | Час на затворање |
...
| **MVC** (Model-View-Controller / Модел-Поглед-Контролор) | Архитектурен образец за дизајн на софтвер кој ја раздвојува апликацијата на три компоненти: модел (податоци), поглед (интерфејс) и контролор (логика). Проектот е инспириран од овој пристап. |
